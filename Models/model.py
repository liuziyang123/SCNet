...
#from .pur_erf import Net
from .pur_erf import Channel_Attn
...
class uncertainty_net(nn.Module):
    def __init__(self, in_channels, out_channels=1, thres=15):
        ...
        # pretrained_model = resnet.__dict__['resnet{}'.format(34)](pretrained=resnet_pretrained)
        ...
        self.convbnrelu = nn.Sequential(convbn(local_channels_in, 32*scale, 3, 1, 1, 1),
                                        nn.ReLU(inplace=True))
        
        
        self.hourglass1 = hourglass_1(32*scale)
        self.hourglass2 = hourglass_2(32*scale)
        self.fuse = nn.Sequential(convbn(32*scale, 32*scale, 3, 1, 1, 1),
                                   nn.ReLU(inplace=True),
                                   nn.Conv2d(32*scale, out_chan, kernel_size=3, padding=1, stride=1, bias=True))
                                   
        print("===========================> init the uncertainty_net")
        define_init_weights(self.convbnrelu,'kaiming')
        define_init_weights(self.hourglass1,'kaiming')
        define_init_weights(self.hourglass2,'kaiming')
        define_init_weights(self.fuse,'kaiming')
    ...
    def forward(self, input, epoch=50):
        ...
        if self.combine == 'concat':
            ...
            #input = torch.cat((input, global_features), 1)
...
class hourglass_1(nn.Module):
    def __init__(self, channels_in):
        ...
        self.channelEnhancer = Channel_Attn(channels_in * 2)
    ...
    def forward(self, x, em1, em2):
        ...
        x_prime = self.channelEnhancer(x_prime) + x_prime
...
class hourglass_2(nn.Module):
    def __init__(self, channels_in):
        ...
        self.channelEnhancer = Channel_Attn(channels_in*4)
