...
parser.add_argument('--nepochs', type=int, default=120, help='Number of epochs for training')
...
parser.add_argument('--directly_path', default='', help='model path')
...
def main():
    ...
    if args.directly_path != '':
        path = args.directly_path
        if os.path.isfile(path):
            ...
            for k, v in pretrain_dict.items():
                key_new = 'module.'+ k
                if key_new in state_dict:
                    if v.size() == state_dict[key_new].size():
                        print('loading: ',key_new)
                        model_dict[key_new] = v
                    else:
                        print('error:',key_new)
                else:
                    print('error:',key_new)
    ...
    for epoch in range(args.start_epoch, args.nepochs):
        ...
        for i, (input, gt) in tqdm(enumerate(train_loader)):
            ...
            if not args.no_tb:
                total_iter += 1
                if total_iter % 10 == 0:
                    writer.add_scalar('loss/lidar',loss_lidar,total_iter)
                    writer.add_scalar('loss/rgb',loss_rgb,total_iter)
                    writer.add_scalar('loss/guide',loss_guide,total_iter)
                    writer.add_scalar('loss/total',loss,total_iter)
